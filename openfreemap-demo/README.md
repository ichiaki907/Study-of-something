...
バス停は数が多く、縮尺を引いた状態では名称だけで画面が埋まってしまうため、
駅・空港とは別レイヤーに分けている（`separateBusStops()`）。

- 元の `poi_transit` は airport / rail のみに絞る（駅はこれまで通り）
- バス停は複製したレイヤーで描画し、**名称は非表示**・**minzoom を 17** に上げる

レイヤー定義は手で書き写さず実行時に複製しているので、アイコンやサイズなど
元の見た目をそのまま引き継ぎ、スタイルごとの差異にも追従する。
...
道路番号のシールドを Google マップ風にしている
（`src/map/roadShields.ts`）。

| 種別 | デザイン |
| --- | --- |
| 国道 | 青地・白フチの**盾型**（上が角丸、下がすぼまる） |
| 県道 | 青地・白フチの**横長六角形**（左右がとがる） |

日本の実際の道路標識（鋭い逆三角形の「おにぎり」／縦長の「ヘキサ」）ではなく、
Google マップが日本で使っている、角を丸めた同系統の形に寄せている。

OpenFreeMap 既定は `road_<桁数>` という白い角丸長方形のスプライトを使う。
スプライトは OpenFreeMap 側にあり差し替えられないため、Canvas で描いた画像を
`map.addImage()` で登録し、レイヤーの icon-image を差し替えている。

### 判別方法の制約（重要）

OpenMapTiles の `network` フィールドは us-* / ca-* / gb-* / ie-* しか値を持たず、
**日本の道路はすべて既定値の "road" になる**。つまりデータから「国道か県道か」を
直接は判別できない。そのため道路種別(`class`)で近似している。

| class | 割り当て |
| --- | --- |
| trunk / primary | 国道 |
| secondary / tertiary | 県道 |
| motorway ほか | 既定のまま（変更なし） |
...
Google マップのスクリーンショットから標識を実測し、同じ端末で撮った本アプリの
スクショと突き合わせて CSS px に換算した値を使っている。

| 項目 | 実測値 |
| --- | --- |
| 塗りの青 | `rgb(72,118,191)` = `#4876bf` |
| 白フチ | 約 1.5px |
| 国道（3桁） | 塗り 約 24 x 18px |
| 県道（3桁） | 塗り 約 26 x 14.5px |
| 数字 | 高さ・送り幅とも本アプリ（text-size 10）とほぼ同じ |

高さは桁数によらず一定で、幅だけが桁数に応じて伸びる（Google も同じ挙動）。
盾型は下半分がすぼまるので、数字は `text-offset` で 0.12em ぶん上に寄せている。

輪郭のベジエ制御点も実測プロファイル（62x47px の標識を1行ずつ走査した
左右端の座標）に当てて決めており、ヘッドレスブラウザで描画して
Google の標識と並べ、形と番号の収まりを確認している。
文字は逆三角形の広い部分へ寄せるため `text-offset` で上に 0.3em ずらしている。
