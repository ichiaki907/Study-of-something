...
道路番号のシールドを日本の実際の道路標識風にしている
（`src/map/roadShields.ts`）。

| 種別 | デザイン |
| --- | --- |
| 国道 | 青地・白フチの**逆三角形**（通称「おにぎり」） |
| 県道 | 青地・白フチの**六角形**（通称「ヘキサ」） |
...
逆三角形は下に向かって細くなるため、文字を置く高さでの内幅が文字幅を
上回るようにする必要がある。ヘッドレスブラウザで実際に描画し、
番号がはみ出さないことを確認して寸法を決めている。
